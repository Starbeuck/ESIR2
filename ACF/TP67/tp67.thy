...
fun Sumquelquechose:: "(int * int) \<Rightarrow> (int * int)  \<Rightarrow> (int * int)"
where
"Sumquelquechose (a,b) (c, d)= (if ((a=-1 \<and> b=-1) \<or> (c=-1 \<and> d=-1)) then (-1,-1) else ((a+c),(b+d)))"

(* override de la soustraction*) 
fun Sousquelquechose:: "(int * int) \<Rightarrow> (int * int)  \<Rightarrow> (int * int)"
where
"Sousquelquechose (a,b) (c, d)= (if ((a=-1 \<and> b=-1) \<or> (c=-1 \<and> d=-1)) then (-1,-1) else ((a-c),(b-d)))"


fun rendTuple::"quelquechose \<Rightarrow> ( int *int)"
where
"rendTuple (Tuple a b) = (a, b)"|
"rendTuple Indefini = (-1, -1)"

(* Recherche la presence d'une variable dans un tableau d'associations *)
fun AssocAb::"string \<Rightarrow> symTablebis \<Rightarrow> (int*int)"
where
"AssocAb _ [] = (-1,-1)"|
"AssocAb a ((b,c)#xs) = (if a=b then (rendTuple c) else (AssocAb a xs))"

(* override de l'evaluation *)
(* Evaluation des expressions par rapport a une table de symboles *)
fun evalEs:: "expression \<Rightarrow> symTablebis \<Rightarrow> (int*int)"
where
"evalEs (Constant s) e = (s,s)" |
"evalEs (Variable s) e= AssocAb s e" |
...
"san4 (If _ s1 s2) st = (let(a, b) = (san4 s1 st) in (if b=False then (a, False) else 
               ...
               (a, True)))))" |              
"san4 (Exec e) st = (if (evalEs e st) = (-1,-1) then (st, False) else (st,True))" |
"san4 (Aff var val) st = (if (evalEs val st) = (-1,-1) then (st, False) else (st,True))" |
